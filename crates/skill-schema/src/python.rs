...
use crate::{
    ParseError, models::*, parse_skill_md as rs_parse_skill_md, scan_and_load as rs_scan_and_load,
};
use std::path::Path;
...
pub fn parse_skill_md(path: &str) -> PyResult<Option<PySkillSpec>> {
    ...
    match rs_parse_skill_md(p) {
        ...
        Err(ParseError::Yaml(e)) => Err(pyo3::exceptions::PyValueError::new_err(format!(
            "SKILL.md YAML parse error: {e}"
        ))),
        Err(ParseError::Io(p, e)) => Err(pyo3::exceptions::PyIOError::new_err(format!(
            "IO error reading {}: {e}",
            p.display()
        ))),
    }
